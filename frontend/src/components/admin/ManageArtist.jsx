    ...
    const handleDelete = async (artist) => {
        if (window.confirm(`Delete ${artist.name}? This action cannot be undone.`)) {
            ...
            try {
                ...
                setArtists((currentArtists) => currentArtists.filter(({ id }) => id !== artist.id));
            } catch (error) {
        ...
        <div className="min-h-screen bg-gray-950">
            ...
            <div className="bg-gray-900 border-b border-gray-800 sticky top-0 z-40">
                <div className="container mx-auto px-4 py-4">
                    <div className="flex items-center justify-between">
                        <div>
                            <h1 className="text-3xl font-black text-white">Manage Roster</h1>
                            <p className="text-gray-400 mt-1">Manage Hip Hop Roster</p>
                        </div>
                        <button
                            onClick={() => navigate('/')}
            ...
            <div className="container mx-auto px-4 py-8">
                ...
                <div className="mb-8 flex justify-between items-center flex-wrap gap-4">
                    <h2 className="text-2xl font-bold text-white">Artists ({artists.length})</h2>
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full">
                            ...
                            <tbody className="divide-y divide-gray-800">
                                {artists.map((artist) => (
                                    <tr key={artist.id} className="hover:bg-gray-800/50 transition">
            ...
            {/* Artist Form Modal */}
            {showForm && <ArtistForm artist={editingArtist} onClose={handleFormClose} />}

            {/* Bulk Import Modal */}
            {showBulkImport && (
                <BulkImport onClose={handleBulkImportClose} onSuccess={handleBulkImportClose} />
            )}
        </div>
    );
};
